  ...
  pageDefs: {
    ...
    dialog: {
     ...
     table: {
      section: 'Components',
      title: 'Table',
      description: 'A data table with sortable columns, multiple visual variants, row selection, and built-in auto search with debounce. Supports borderless, headless, fixed layout, and clickable rows.',
      guidance: [
        'Table supports sortable columns — delegate sort state up when the data comes from a server.',
        'Use variant="dark" for emphasis or variant="outline" for minimal chrome.',
        'Enable selectable to add checkbox row selection; control state via selectedRows/onSelectionChange.',
        'Built-in search is debounced (200ms default) and works automatically — no external state needed.',
        'Set searchable: false on a column to exclude it from text search.',
      ],
      props: [
        ...
        { prop: 'stickyFilter', type: 'boolean', default: 'false', description: 'Sticky filter bar, keeps search box visible when scrolling.' },
        ...
        { prop: 'variant', type: '"default" | "dark" | "outline"', default: '"default"', description: 'Visual style variant.' },
        { prop: 'fixed', type: 'boolean', default: 'false', description: 'Fixed table layout (column widths honored strictly, text truncated).' },
        { prop: 'removeBorders', type: '"all" | ("top" | "right" | "bottom" | "left")[]', description: 'Remove specific side borders or all. Adjusts border-radius accordingly.' },
        { prop: 'flush', type: 'boolean', default: 'false', description: 'Flush mode: removes border, border-radius, and padding for embedding inside Card.' },
        { prop: 'onRowClick', type: '(row: T, index: number) => void', description: 'Row click callback.' },
        { prop: 'rowClassName', type: 'string | ((row: T, index: number) => string)', description: 'Custom row class name.' },
        { prop: 'selectable', type: 'boolean', default: 'false', description: 'Enable checkbox row selection.' },
        { prop: 'selectedRows', type: 'Set<number>', description: 'Controlled selected row indices.' },
        { prop: 'onSelectionChange', type: '(selected: Set<number>) => void', description: 'Selection change callback.' },
        { prop: 'searchDebounce', type: 'number', default: '200', description: 'Search debounce delay in ms. Set 0 to disable.' },
        { prop: 'searchColumns', type: 'string[]', description: 'Limit text search to specific column keys.' },
    ...
    image: {
      section: 'Content',
      title: 'Image',
      description: 'A progressive image component with lazy loading via IntersectionObserver, error fallback, and an optional click-to-preview lightbox. Supports fixed dimensions, border-radius presets, and captions.',
      guidance: [
        'Set lazy={true} (default) to defer off-screen images and improve initial load time.',
        'Provide a fallback node to replace the default placeholder when the image fails to load.',
        'Enable preview={true} for gallery-style layouts where users expect to inspect images.',
        'Use width and height props to prevent layout shift (CLS) during loading.',
      ],
      props: [
        ...
        { prop: 'width', type: 'string | number', description: 'Fixed width (CSS value or px number).' },
        { prop: 'height', type: 'string | number', description: 'Fixed height (CSS value or px number).' },
        { prop: 'radius', type: "'none' | 'sm' | 'md' | 'lg' | 'full'", default: "'md'", description: 'Border radius preset.' },
        { prop: 'fit', type: "'cover' | 'contain' | 'fill' | 'none'", default: "'cover'", description: 'Object-fit CSS value.' },
        { prop: 'lazy', type: 'boolean', default: 'true', description: 'Enable lazy loading.' },
        { prop: 'placeholder', type: 'ReactNode', description: 'Placeholder shown while loading.' },
        { prop: 'fallback', type: 'ReactNode', description: 'Fallback shown on error.' },
        { prop: 'preview', type: 'boolean', default: 'false', description: 'Enable click-to-preview lightbox.' },
        { prop: 'caption', type: 'ReactNode', description: 'Caption text below the image.' },
      ],
    },
    'pin-input': {
      section: 'Forms',
      title: 'Pin Input',
      description: 'A segmented input for verification codes, OTPs, and PINs. Supports numeric or alphanumeric modes, auto-advance, paste handling, and mask mode.',
      guidance: [
        'Use length={6} for standard verification codes or length={4} for PINs.',
        'Set type="numeric" (default) for number-only codes; use "alphanumeric" for mixed.',
        'Provide an onComplete callback to auto-submit once all fields are filled.',
        'Enable mask={true} for sensitive codes that should not be visible.',
      ],
      props: [
        { prop: 'length', type: 'number', default: '6', description: 'Number of input fields.' },
        { prop: 'value', type: 'string', description: 'Controlled value.' },
        { prop: 'onChange', type: '(value: string) => void', description: 'Called when any field changes.' },
        { prop: 'onComplete', type: '(value: string) => void', description: 'Called when all fields are filled.' },
        { prop: 'type', type: "'numeric' | 'alphanumeric'", default: "'numeric'", description: 'Input type.' },
        { prop: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'", description: 'Size preset.' },
        { prop: 'mask', type: 'boolean', default: 'false', description: 'Mask input (show dots).' },
        { prop: 'error', type: 'string', description: 'Error message below the input group.' },
        { prop: 'label', type: 'string', description: 'Label above the input group.' },
        { prop: 'autoFocus', type: 'boolean', default: 'false', description: 'Auto-focus the first input.' },
      ],
    },
    descriptions: {
      section: 'Content',
      title: 'Descriptions',
      description: 'A key-value display component for detail pages. Organizes metadata in a structured horizontal or vertical layout with optional borders, column spanning, and size presets.',
      guidance: [
        'Use horizontal layout for wide screens; vertical for narrow or single-column details.',
        'Set column={2} or column={3} to arrange items in multi-column rows.',
        'Use the span property on individual items to stretch across multiple columns.',
        'Combine with the title and extra props for a section header with actions.',
      ],
      props: [
        { prop: 'items', type: 'DescriptionsItem[]', required: true, description: 'List of description items.' },
        { prop: 'layout', type: "'horizontal' | 'vertical'", default: "'horizontal'", description: 'Layout direction.' },
        { prop: 'column', type: 'number', default: '3', description: 'Number of columns per row.' },
        { prop: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'", description: 'Size preset.' },
        { prop: 'bordered', type: 'boolean', default: 'true', description: 'Show border around the descriptions.' },
        { prop: 'title', type: 'ReactNode', description: 'Title above the descriptions.' },
        { prop: 'extra', type: 'ReactNode', description: 'Extra content next to the title.' },
      ],
    },
    notification: {
      section: 'Components',
      title: 'Notification',
      description: 'A persistent notification system with NotificationProvider and useNotification hook. Unlike Toast (auto-dismiss), notifications remain visible until dismissed manually or via duration.',
      guidance: [
        'Wrap your app with NotificationProvider and set the desired placement.',
        'Use push() to show a notification — it returns an id for programmatic dismiss.',
        'Set duration > 0 for auto-dismiss; use 0 (default) for persistent notifications.',
        'Use the action prop for inline buttons like "View details" or "Undo".',
      ],
      props: [
        { prop: 'NotificationProvider.placement', type: "'top-right' | 'top-left' | 'top-center' | 'bottom-right' | 'bottom-left' | 'bottom-center'", default: "'top-right'", description: 'Stack position.' },
        { prop: 'NotificationProvider.maxCount', type: 'number', default: '5', description: 'Maximum visible notifications.' },
        { prop: 'push', type: '(input: NotificationInput) => number', description: 'Show a notification; returns the id.' },
        { prop: 'dismiss', type: '(id: number) => void', description: 'Dismiss a notification by id.' },
        { prop: 'clear', type: '() => void', description: 'Dismiss all notifications.' },
      ],
    },
    result: {
      section: 'Components',
      title: 'Result',
      description: 'A result/status page component for operation feedback. Provides pre-built status presets (success, error, warning, info, forbidden, not-found, server-error) with default icons and colors.',
      guidance: [
        'Use the status prop for common outcomes; override icon for custom branding.',
        'Always provide a clear next action via the actions prop (e.g., "Back to Home").',
        'Keep the description to one or two sentences explaining the result.',
        'Use the extra slot for supplementary content like order details or error logs.',
      ],
      props: [
        { prop: 'status', type: "'success' | 'error' | 'warning' | 'info' | 'forbidden' | 'not-found' | 'server-error'", required: true, description: 'Pre-built status preset.' },
        { prop: 'title', type: 'string', required: true, description: 'Title text.' },
        { prop: 'description', type: 'string', description: 'Description below the title.' },
        { prop: 'icon', type: 'ReactNode', description: 'Custom icon (overrides default).' },
        { prop: 'actions', type: 'ReactNode', description: 'Action area for buttons/links.' },
        { prop: 'extra', type: 'ReactNode', description: 'Extra content below actions.' },
      ],
    },
    'home-page': {
      section: 'Templates',
...
export const zh: Translations = {
  ...
  pageDefs: {
    ...
    table: {
      section: '组件',
      title: '表格',
      description: '支持可排序列、多种视觉变体、行选择和内置防抖自动搜索的数据表格。支持无边框、无头部、固定布局和可点击行。',
      guidance: [
        'Table 支持可排序列——后端数据时请将排序状态向上委托。',
        '使用 variant="dark" 强调视觉，或用 variant="outline" 极简风格。',
        '开启 selectable 添加复选框行选择，通过 selectedRows/onSelectionChange 控制状态。',
        '内置搜索自动防抖（默认 200ms），无需外部状态管理。',
        '对列设置 searchable: false 可将其排除在文本搜索之外。',
      ],
      props: [
        ...
        { prop: 'bordered', type: 'boolean', default: 'false', description: '单元格内边框。' },
        { prop: 'stickyHeader', type: 'boolean', default: 'false', description: '固定表头。' },
        { prop: 'stickyFilter', type: 'boolean', default: 'false', description: '搜索栏吸顶，滚动时保持搜索框可见。' },
        { prop: 'borderless', type: 'boolean', default: 'false', description: '移除所有边框（包括外层包裹器边框）。' },
        { prop: 'headless', type: 'boolean', default: 'false', description: '隐藏表头行。' },
        { prop: 'loading', type: 'boolean', default: 'false', description: '显示加载遮罩。' },
        { prop: 'variant', type: '"default" | "dark" | "outline"', default: '"default"', description: '视觉风格变体。' },
        { prop: 'fixed', type: 'boolean', default: 'false', description: '固定表格布局（列宽严格执行，文本截断不换行）。' },
        { prop: 'removeBorders', type: '"all" | ("top" | "right" | "bottom" | "left")[]', description: '移除指定边边框或四边全部移除，同步调整圆角。' },
        { prop: 'flush', type: 'boolean', default: 'false', description: '嵌入模式：移除外边框、圆角和内边距，适合直接放入 Card 等容器。' },
        { prop: 'onRowClick', type: '(row: T, index: number) => void', description: '行点击回调。' },
        { prop: 'rowClassName', type: 'string | ((row: T, index: number) => string)', description: '自定义行类名。' },
        { prop: 'selectable', type: 'boolean', default: 'false', description: '启用复选框行选择。' },
        { prop: 'selectedRows', type: 'Set<number>', description: '受控选中行索引集合。' },
        { prop: 'onSelectionChange', type: '(selected: Set<number>) => void', description: '选中行变化回调。' },
        { prop: 'searchDebounce', type: 'number', default: '200', description: '搜索防抖延迟（毫秒），设为 0 禁用。' },
        { prop: 'searchColumns', type: 'string[]', description: '限定文本搜索的列 key 列表。' },
    ...
    image: {
      section: '内容展示',
      title: '图片',
      description: '支持 IntersectionObserver 懒加载、错误回退和可选点击预览灯箱的图片组件。支持固定尺寸、圆角预设和说明文字。',
      guidance: [
        '设置 lazy={true}（默认）延迟屏幕外图片加载，提升初始加载速度。',
        '提供 fallback 节点在图片加载失败时替换默认占位符。',
        '启用 preview={true} 适用于用户需要查看大图的画廊式布局。',
        '使用 width 和 height 属性防止加载期间的布局偏移（CLS）。',
      ],
      props: [
        { prop: 'src', type: 'string', required: true, description: '图片来源 URL。' },
        { prop: 'alt', type: 'string', description: '无障碍替代文本。' },
        { prop: 'width', type: 'string | number', description: '固定宽度（CSS 值或 px 数字）。' },
        { prop: 'height', type: 'string | number', description: '固定高度（CSS 值或 px 数字）。' },
        { prop: 'radius', type: "'none' | 'sm' | 'md' | 'lg' | 'full'", default: "'md'", description: '圆角预设。' },
        { prop: 'fit', type: "'cover' | 'contain' | 'fill' | 'none'", default: "'cover'", description: 'object-fit CSS 值。' },
        { prop: 'lazy', type: 'boolean', default: 'true', description: '启用懒加载。' },
        { prop: 'placeholder', type: 'ReactNode', description: '加载中显示的占位符。' },
        { prop: 'fallback', type: 'ReactNode', description: '错误时显示的回调内容。' },
        { prop: 'preview', type: 'boolean', default: 'false', description: '启用点击预览灯箱。' },
        { prop: 'caption', type: 'ReactNode', description: '图片下方的说明文字。' },
      ],
    },
    'pin-input': {
      section: '表单',
      title: '验证码输入',
      description: '用于验证码、OTP 和 PIN 的分段输入组件。支持数字或字母模式、自动前进、粘贴处理和掩码模式。',
      guidance: [
        '使用 length={6} 作为标准验证码，length={4} 作为 PIN 码。',
        '设置 type="numeric"（默认）仅输入数字；使用 "alphanumeric" 支持混合输入。',
        '提供 onComplete 回调在所有字段填满时自动提交。',
        '启用 mask={true} 用于不应可见的敏感码。',
      ],
      props: [
        { prop: 'length', type: 'number', default: '6', description: '输入字段数量。' },
        { prop: 'value', type: 'string', description: '受控值。' },
        { prop: 'onChange', type: '(value: string) => void', description: '任意字段变化时调用。' },
        { prop: 'onComplete', type: '(value: string) => void', description: '所有字段填满时调用。' },
        { prop: 'type', type: "'numeric' | 'alphanumeric'", default: "'numeric'", description: '输入类型。' },
        { prop: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'", description: '尺寸预设。' },
        { prop: 'mask', type: 'boolean', default: 'false', description: '掩码输入（显示圆点）。' },
        { prop: 'error', type: 'string', description: '输入组下方的错误信息。' },
        { prop: 'label', type: 'string', description: '输入组上方的标签。' },
        { prop: 'autoFocus', type: 'boolean', default: 'false', description: '自动聚焦第一个输入框。' },
      ],
    },
    descriptions: {
      section: '内容展示',
      title: '描述列表',
      description: '用于详情页的键值对展示组件。以结构化的水平或垂直布局组织元数据，支持可选边框、跨列和尺寸预设。',
      guidance: [
        '宽屏使用水平布局；窄屏或单列详情使用垂直布局。',
        '设置 column={2} 或 column={3} 将项目排成多列行。',
        '使用单项的 span 属性跨越多列。',
        '结合 title 和 extra 属性实现带操作的区块标题。',
      ],
      props: [
        { prop: 'items', type: 'DescriptionsItem[]', required: true, description: '描述项列表。' },
        { prop: 'layout', type: "'horizontal' | 'vertical'", default: "'horizontal'", description: '布局方向。' },
        { prop: 'column', type: 'number', default: '3', description: '每行列数。' },
        { prop: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'", description: '尺寸预设。' },
        { prop: 'bordered', type: 'boolean', default: 'true', description: '显示边框。' },
        { prop: 'title', type: 'ReactNode', description: '描述列表上方的标题。' },
        { prop: 'extra', type: 'ReactNode', description: '标题旁的额外内容。' },
      ],
    },
    notification: {
      section: '组件',
      title: '通知',
      description: '持久化通知系统，包含 NotificationProvider 和 useNotification hook。与 Toast（自动消失）不同，通知保持可见直到手动关闭或设置 duration。',
      guidance: [
        '用 NotificationProvider 包裹应用并设置期望的位置。',
        '使用 push() 显示通知——它返回一个 id 用于程序化关闭。',
        '设置 duration > 0 自动消失；使用 0（默认）持久显示。',
        '使用 action 属性放置内联按钮如“查看详情”或“撤销”。',
      ],
      props: [
        { prop: 'NotificationProvider.placement', type: "'top-right' | 'top-left' | 'top-center' | 'bottom-right' | 'bottom-left' | 'bottom-center'", default: "'top-right'", description: '堆叠位置。' },
        { prop: 'NotificationProvider.maxCount', type: 'number', default: '5', description: '最大可见通知数。' },
        { prop: 'push', type: '(input: NotificationInput) => number', description: '显示通知；返回 id。' },
        { prop: 'dismiss', type: '(id: number) => void', description: '按 id 关闭通知。' },
        { prop: 'clear', type: '() => void', description: '关闭所有通知。' },
      ],
    },
    result: {
      section: '组件',
      title: '结果页',
      description: '用于操作反馈的结果/状态页组件。提供预设状态（success、error、warning、info、forbidden、not-found、server-error）及默认图标和颜色。',
      guidance: [
        '使用 status 属性处理常见结果；自定义 icon 属性实现品牌化。',
        '始终通过 actions 属性提供清晰的下一步操作（如“返回首页”）。',
        'description 保持一两句话说明结果。',
        '使用 extra 插槽放置补充内容，如订单详情或错误日志。',
      ],
      props: [
        { prop: 'status', type: "'success' | 'error' | 'warning' | 'info' | 'forbidden' | 'not-found' | 'server-error'", required: true, description: '预置状态。' },
        { prop: 'title', type: 'string', required: true, description: '标题文字。' },
        { prop: 'description', type: 'string', description: '标题下方的描述。' },
        { prop: 'icon', type: 'ReactNode', description: '自定义图标（覆盖默认）。' },
        { prop: 'actions', type: 'ReactNode', description: '按钮/链接操作区。' },
        { prop: 'extra', type: 'ReactNode', description: '操作区下方的额外内容。' },
      ],
    },
    'home-page': {
      section: '模板',
